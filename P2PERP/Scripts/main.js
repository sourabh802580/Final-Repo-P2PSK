...
let menu, animate, isHorizontalLayout = !1, SearchConfig = (document.getElementById("layout-menu") && (isHorizontalLayout = document.getElementById("layout-menu").classList.contains("menu-horizontal")),
    ...
    (() => {
        ...
        var n = localStorage.getItem("templateCustomizer-" + templateName + "--Theme") || (window.templateCustomizer?.settings?.defaultStyle ?? document.documentElement.getAttribute("data-bs-theme"));
        function o() {
            var e = window.innerWidth - document.documentElement.clientWidth;
            document.body.style.setProperty("--bs-scrollbar-width", e + "px")
        }
        if (window.Helpers.switchImage(n),
                ...
                window.Helpers.showActiveTheme(window.Helpers.getPreferredTheme()),
                    ...
                    document.querySelectorAll("[data-bs-theme-value]").forEach(n => {
                        n.addEventListener("click", () => {
                            ...
                            a && ("dark" === e ? a.classList.add("d-none") : a.classList.remove("d-none")),
            ...
            (n = document.getElementsByClassName("dropdown-language")).length) {
                    ...
                    this.classList.add("active"),
                        i18next.changeLanguage(n, (e, t) => {
                            ...
                            if (window.templateCustomizer && window.templateCustomizer.setLang(n),
                                ...
                                "rtl" === a ? "true" !== localStorage.getItem("templateCustomizer-" + templateName + "--Rtl") && window.templateCustomizer && window.templateCustomizer.setRtl(!0) : "true" === localStorage.getItem("templateCustomizer-" + templateName + "--Rtl") && window.templateCustomizer && window.templateCustomizer.setRtl(!1),
                                e)
        ...
        function l(e) {
            "show.bs.collapse" == e.type || "show.bs.collapse" == e.type ? (e.target.closest(".accordion-item").classList.add("active"),
                e.target.closest(".accordion-item").previousElementSibling?.classList.add("previous-active")) : (e.target.closest(".accordion-item").classList.remove("active"),
        ...
        let r = document.querySelectorAll(".dropdown-notifications-read")
            , d = (n && n.addEventListener("click", e => {
                r.forEach(e => {
...
function initializeAutocomplete() {
    if (document.getElementById("autocomplete"))
        return autocomplete({
            ...
            onStateChange({ state: e, setQuery: t }) {
                ...
                e.isOpen ? (document.body.style.overflow = "hidden",
                    ...
                    (a = document.querySelector(".aa-DetachedCancelButton")) && (a.innerHTML = '<span class="text-body-secondary">[esc]</span> <span class="icon-base icon-md ri ri-close-line text-heading"></span>'),
                    window.autoCompletePS || (a = document.querySelector(".aa-Panel")) && (window.autoCompletePS = new PerfectScrollbar(a))) : ("idle" === e.status && e.query && t(""),
                        document.body.style.overflow = "auto",
                        document.body.style.paddingRight = "")
            },
            render(e, t) {
                let { render: a, html: n, children: o, state: s } = e;
                s.query ? e.sections.length ? (a(o, t),
                    window.autoCompletePS?.update()) : a(n`
        ...
        `,
            ...
            getSources() {
                var e, t = [];
                return data.navigation && (e = Object.keys(data.navigation).filter(e => "files" !== e && "members" !== e).map(a => ({
                    sourceId: "nav-" + a,
                    getItems({ query: t }) {
                    ...
                    templates: {
                        header({ items: e, html: t }) {
                            return 0 === e.length ? null : t`<span class="search-headings">${a}</span>`
                        },
                        item({ item: e, html: t }) {
                            return t`
